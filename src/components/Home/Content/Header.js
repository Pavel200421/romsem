            ...
            <div className='header__right'>
                <ul className='header__list'>
                    <li className='header__list-item'>
                        <Link className='header__link' to="/reviews">Отзывы</Link>
                    </li>
                    <li className='header__list-item'>
                        <Link className='header__link' to="/Delivery">Доставка и оплата</Link>
                    </li>
                </ul>
                <div className='header__search'>
                    <input style={{display: `${search ? 'inline-block' : 'none'}`}} type="search" className='header__search-input' onChange={(e)=> searchHandler(e.target.value)}/>
                    ...
                    <ul style={{display: `${search && all.length ? 'flex' : 'none'}`}}  className='header__search-list'>
                        {all.map((item, idx)=> (
                            <li key={idx} className='header__search-list-item'>
                                <img className='header__search-list-item-img' src={item.imageUrl} alt="pizza"/>
                                <div className='header__search-list-item-info'>
                                    <h2 className='header__search-list-item-title'>{item.title}</h2>
                                    <p className='header__search-list-item-subtitle'>{item.price} Сом</p>
                                </div>
                            </li>
                        ))}
                    </ul>
